...
def load_weights_koopman(fname):

    params = get_params_from_file(fname)
    weights, biases = load_weights(fname, len(params['encoder_widths'])-1, 'E')
    
    W, b = load_weights(fname, len(params['decoder_widths'])-1, 'D')
    weights.update(W)
    biases.update(b)

    W, b = load_weights(fname, len(params['k_widths'])-1, 'K')
    weights.update(W)
    biases.update(b)
...
def create_dummy_koopman_net(fname):
    ...
    enc_layers = []
    for i in range(len(params['encoder_widths'])-1):
        if i == 0:
            enc_layers.append(tf.keras.layers.Dense(params['encoder_widths'][i+1], 
                                            use_bias=params['use_bias'], 
                                            activation=params['encoder_act_type'],
                                            name=f'encoder_{i+1}')(input_layer))
        else:
            enc_layers.append(tf.keras.layers.Dense(params['encoder_widths'][i+1], 
                                            use_bias=params['use_bias'], 
                                            activation=params['encoder_act_type'],
                                            name=f'encoder_{i+1}')(enc_layers[i-1]))

    k_layers = []
    for i in range(1, len(params['k_widths'])):
        if i == 1:
            k_layers.append(tf.keras.layers.Dense(params['k_widths'][i], 
                                            use_bias=False, 
                                            activation='linear',
                                            name=f'k_{i}')(enc_layers[-1]))
        else:
            k_layers.append(tf.keras.layers.Dense(params['k_widths'][i], 
                                            use_bias=False, 
                                            activation='linear',
                                            name=f'k_{i}')(k_layers[i-2]))

    dec_layers = []
    for i in range(1, len(params['decoder_widths'])):
        if i == 1:
            dec_layers.append(tf.keras.layers.Dense(params['decoder_widths'][i], 
                                            use_bias=params['use_bias'], 
                                            activation=params['decoder_act_type'],
                                            name=f'decoder_{i}')(k_layers[-1]))
        else:
            dec_layers.append(tf.keras.layers.Dense(params['decoder_widths'][i], 
                                            use_bias=params['use_bias'], 
                                            activation=params['decoder_act_type'],
                                            name=f'decoder_{i}')(dec_layers[i-2]))

    model = tf.keras.Model(inputs=input_layer, outputs=dec_layers[-1])
...
def set_koopman_net_weights(fname, model):
