...
def choose_optimizer(params, regularized_loss, trainable_var):
    ...
    if params['opt_alg'] == 'adam':
        ...
        optimizer = tf.compat.v1.train.AdagradDAOptimizer(params['learning_rate'], tf.compat.v1.train.get_global_step()).minimize(
            regularized_loss,
            var_list=trainable_var)
...
def set_defaults(params):
    ...
    if 'encoder_widths' not in params:
        raise KeyError("Error, must give encoder_widths as input to main")
    if 'decoder_widths' not in params:
        raise KeyError("Error, must give decoder_widths as input to main")
    if 'k_widths' not in params:
        raise KeyError("Error, must give k_widths as input to main")
    print(params['encoder_widths'], params['decoder_widths'], params['k_widths'])
    
    if 'encoder_act_type' not in params:
