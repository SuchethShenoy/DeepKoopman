...
import simple_networkarch as net
...
def define_loss(x, y, g_list, weights, biases, params):
    ...
    Arguments:
        ...
        params -- dictionary of parameters for experiment
            ...
            shifts -- shifts to evaluate prediction loss, list
                num_shifts -- length of shifts
            shifts_middle -- shifts to evaluate linearity loss, list
                num_shifts_middle -- length of shifts_middle
    ...
    if params['relative_loss']:
        ...
        loss1_denominator = tf.compat.v1.to_double(1.0)
    ...
    if params['num_shifts'] > 0:
        for j in np.arange(params['num_shifts']):
            ...
            if params['relative_loss']:
                ...
                loss2_denominator = tf.compat.v1.to_double(1.0)
    ...
    if params['num_shifts_middle'] > 0:
        ...
        next_step = net.k_block_apply_one_shift(g_list[0], weights)
        ...
        for j in np.arange(max(params['shifts_middle'])):
            if (j + 1) in params['shifts_middle']:
                if params['relative_loss']:
                    ...
                    loss3_denominator = tf.compat.v1.to_double(1.0)
            ...
            next_step = net.k_block_apply_one_shift(next_step, weights)
    ...
    if params['relative_loss']:
        ...
        Linf1_den = tf.compat.v1.to_double(1.0)
        Linf2_den = tf.compat.v1.to_double(1.0)
...
def define_regularization(params, trainable_var, loss, loss1):
    ...
    """
    if params['L1_lam']:
        l1_regularizer = slim.l1_regularizer(scale=params['L1_lam'], scope=None)
        # TODO: don't include biases? use weights dict instead?
        loss_L1 = slim.apply_regularization(l1_regularizer, weights_list=trainable_var)
    else:
...
def try_net(data_val, params):
    ...
    optimizer = simple_helperfns.choose_optimizer(params, regularized_loss, trainable_var)
    optimizer_autoencoder = simple_helperfns.choose_optimizer(params, regularized_loss1, trainable_var)
    ...
    data_val_tensor = simple_helperfns.stack_data(data_val, max_shifts_to_stack, params['len_time'])
    ...
    for f in range(params['data_train_len'] * params['num_passes_per_file']):
        ...
        if (params['data_train_len'] > 1) or (f == 0):
            ...
            data_train = np.loadtxt(('./data/%s_train%d_x.csv' % (params['data_name'], file_num)), delimiter=',',
                                    dtype=np.float64)
            data_train_tensor = simple_helperfns.stack_data(data_train, max_shifts_to_stack, params['len_time'])
        ...
        for step in range(params['num_steps_per_batch'] * num_batches):
            ...
            if step % 20 == 0:
                ...
                finished, save_now = simple_helperfns.check_progress(start, best_error, params)
                count = count + 1
                if save_now:
                    train_val_error_trunc = train_val_error[range(count), :]
                    simple_helperfns.save_files(sess, csv_path, train_val_error_trunc, params, weights, biases)
                if finished:
                    break
    ...
    simple_helperfns.save_files(sess, csv_path, train_val_error, params, weights, biases)
...
def main_exp(params):
    ...
    """
    simple_helperfns.set_defaults(params)
